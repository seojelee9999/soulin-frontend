...
## 2026-04-27 BookmarkContext fetch / toggle race

BookmarkContext가 마운트 시 `fetchBookmarks()`로 `bookmarkedIds`를 채우는데, 이 fetch가 pending인 동안 사용자가 글 상세에서 북마크를 토글하면 `bookmarkedIds.has(postId)` 판단이 잘못됨 — 이미 북마크된 글인데 set이 비어있으면 add(POST) 호출 → 백엔드는 중복 추가로 403 → 자동 logout. 핫픽스(2026-04-27)로 add/remove 메서드는 분리됐지만 race 자체는 그대로.

**해결 옵션:**
- fetch pending 동안 토글 버튼 disable (`isLoading` flag 추가)
- 글 상세 진입 시 그 글의 북마크 상태를 응답으로 받아 사용 (백엔드 응답에 `isBookmarked` 필드 요청)
